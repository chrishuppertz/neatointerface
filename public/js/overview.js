...
function createDatePicker() {
    return new mdDateTimePicker.default({ // eslint-disable-line
        type: 'time',
...
function addDateEventListener(label, datePicker, day) {
    ...
    label.addEventListener('onOk', () => {
        ...
        makeRequest('setschedule?param=' + label.day + ' ' + time.getHours() + ' ' + time.getMinutes(), checkOpenRequest);
...
timeText.addEventListener('onOk', () => {
    ...
    makeRequest('settime?param=' + time.getDay() + ' ' + time.getHours() + ' ' + time.getMinutes(), checkOpenRequest);
...
searchBtn.addEventListener('click', () => {
    makeRequest('playsound', checkOpenRequest);
});

// Clean House
cleanHouseBtn.addEventListener('click', () => {
    makeRequest('cleanhouse', checkOpenRequest);
    setTimeout(function () { makeRequest('getcharger', getState); }, 3000);
});

// Clean Spot
cleanSpotBtn.addEventListener('click', () => {
    makeRequest('cleanspot', checkOpenRequest);
    setTimeout(function () { makeRequest('getcharger', getState); }, 3000);
});

// Stop cleaning
stopBtn.addEventListener('click', () => {
    makeRequest('cleanstop', checkOpenRequest);
    setTimeout(function () { makeRequest('getcharger', getState); }, 3000);
});
...
scheduleBtn.addEventListener('click', () => {
    if (scheduleBtn.innerHTML.includes('Disable')) {
        makeRequest('setschedule?param=Off', checkOpenRequest);
    } else if (scheduleBtn.innerHTML.includes('Enable')) {
        makeRequest('setschedule?param=On', checkOpenRequest);
    }
...
function setSchedule(switchElement, day, datePicker) {
    if (switchElement.checked === true) {
        ...
        makeRequest('setschedule?param=' + day + ' 0 0 None', checkOpenRequest);
    }
...
function makeRequest(uri, callback) {
    ...
    xhttp.onerror = error;
    xhttp.open('GET', window.location.href + uri, true);
    xhttp.send();
...
function checkOpenRequest() {
    console.log(openRequest.length);
    openRequest.pop();
    if (openRequest.length === 0) {
        swal.close();
        console.log(openRequest.length);
